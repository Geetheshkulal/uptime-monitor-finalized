    ...
    @hasrole('user')
        <li class="nav-item raiseIssue{{ request()->routeIs('display.tickets') ? 'active' : '' }}">
            <a class="nav-link" href="{{ route('display.tickets') }}">
                ...
                @if ($unreadComments > 0)
                    <span class="badge badge-success ml-2" style="font-size: 10px; padding: 2px 5px;">comments
                        {{ $unreadComments }}</span>
                @endif
            </a>
        </li>
        ...
        @hasrole('user')
            @if ($trialDaysLeft > 0 && auth()->user()->hasRole('user'))
                <li class="nav-item trial-notice mt-2 mb-2">
                    <div class="trial-banner p-2 text-center position-relative"
                        style="border-radius: 6px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);">
                        <div class="trial-badge bg-warning text-dark px-2 py-1 rounded-pill d-inline-block"
                            style="font-size: 0.6rem; position: absolute; top: -8px; left: 50%; transform: translateX(-50%); white-space: nowrap;">

                        </div>
                        <div class="d-flex flex-column align-items-center pt-2">
                            <div class="trial-icon d-flex align-items-center justify-content-center mb-1"
                                style="width: 30px; height: 30px; background: rgba(255,255,255,0.2); border-radius: 50%;">
                                <i class="fas fa-gift" style="font-size: 1rem; color: #e2fb65;"></i>
                            </div>
                            <span class="text-white fw-bold" style="font-size: 0.8rem; line-height: 1.2;">Free Trial
                                Active!</span>
